...
export class DocumentParserService {
	...
	private async downloadFile(url: string): Promise<Buffer> {
		try {
			...
			this.logger.log(`文件下载成功: 大小=${(buffer.length / 1024).toFixed(2)}KB`)
			...
			throw new BadRequestException(`文件下载失败: ${error.message || '未知错误'}`)
		}
	}
	...
	private async parsePdf(buffer: Buffer): Promise<string> {
		try {
			...
			this.logger.log(`PDF 解析成功: 长度=${data.text.length}`)
			...
			throw new BadRequestException(`PDF 文件解析失败: ${error.message}。请确保文件未加密且未损坏`)
		}
	}
	...
	private async parseDocx(buffer: Buffer): Promise<string> {
		try {
			...
			if (result.messages && result.messages.length > 0) {
				this.logger.warn(`DOCX 解析警告: ${result.messages.map((m) => m.message).join(', ')}`)
			}
			...
			throw new BadRequestException(`DOCX 文件解析失败: ${error.message}。请确保文件格式正确且未损坏`)
		}
	}
	...
	validateResumeContent(text: string): {
		...
		const foundKeywords = resumeKeywords.filter((keyword) => text.includes(keyword))

		if (foundKeywords.length < 3) {
			warnings.push('简历内容可能不完整，缺少常见的关键信息（如：姓名、教育、工作经验等）')
		}
